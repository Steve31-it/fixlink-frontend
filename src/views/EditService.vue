<template>
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-gray-900 mb-4">Edit Service</h1>
    ...
    <div v-else-if="!service" class="text-center py-12">
      ...
      <div class="mt-6">
        <router-link to="/my-services" class="btn-primary">
          Back to My Services
        </router-link>
      </div>
    </div>
...
const loadService = async () => {
  try {
    ...
    form.category = service.value.category || ''
    form.description = service.value.description || ''
    form.price = service.value.price || ''
    form.priceType = service.value.priceType || ''
    form.estimatedDuration = service.value.estimatedDuration || ''
    form.serviceArea = service.value.serviceArea || ''
    form.isActive = service.value.isActive !== undefined ? service.value.isActive : true
    form.availability = {
      from: service.value.availability?.from || '',
      to: service.value.availability?.to || '',
      days: service.value.availability?.days || []
    }
  } catch (error) {
